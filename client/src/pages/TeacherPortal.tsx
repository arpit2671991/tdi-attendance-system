...
const handleSubmit = async () => {
  if (!currentSession || !currentUser || !startTime || !endTime) return;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-heading font-bold tracking-tight">My Classes</h1>
        <p className="text-muted-foreground">Welcome, <span className="text-xl font-bold text-black">({currentUser.name}).</span><span> Mark daily attendance below.</span></p>
      </div>
 ...
 {/* Timer */}
  ...
  onClick={handleSubmit} 
  className="w-full sm:w-auto gap-2"
  disabled={createAttendance.isPending || !endTime}
>
  <Save className="h-4 w-4" />
  Save Attendance
</Button>
